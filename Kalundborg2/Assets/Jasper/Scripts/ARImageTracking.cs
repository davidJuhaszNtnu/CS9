...
public class ARImageTracking : MonoBehaviour
{
    ...
    private void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs eventArgs) 
    {
        ...
        { 
            ...
            foreach (var curPrefab in ArPrefabs) //For loop checking if detected image matches with a model inside ArPrefabs
            { 
                if (imageName == curPrefab.name && !_instantiatedPrefabs.ContainsKey(imageName)) // If model name matches image name we should instantiate that model above the image
                {   
